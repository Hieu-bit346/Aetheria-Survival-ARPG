...
// Nạp ảnh đứng yên
ASSETS.playerIdle.src = 'MainCharacter.png';

// Nạp bộ ảnh di chuyển của Lucas
for(let i = 1; i <= 4; i++) { 
    let img = new Image(); 
    img.src = `Move${i}.png`; 
...
for(let i = 1; i <= 2; i++) { 
    let img = new Image(); 
    img.src = `Attack${i}.png`; 
    ASSETS.playerAttack.push(img); 
}

// Nạp ảnh lướt
ASSETS.playerDash.src = 'Dash.png';

// Nạp ảnh hệ thống kỹ năng nguyên tố, Hiệu ứng chém & Các Icon Trạng thái
const skillImages = {
    normalSlash: 'NormalAttack.png',
    fireBall: 'Fire_Ball.png', fireExplosion: 'Fire_Explosion.png', fireZone: 'Fire_Zone.png', fireSlash: 'Fire_Slash.png',
    waterWave: 'Water_Wave.png', waterBall: 'Water_Ball.png', mudPuddle: 'Mud_Puddle.png',
    tornado: 'Tornado.png', windSlash: 'Wind_Slash.png', dustStorm: 'Dust_Storm.png',
    earthSpike: 'Earth_Spike.png', earthSmash: 'Earth_Smash.png', rockFragment: 'Rock_Fragment.png',
    vfxThermal: 'VFX_Thermal.png', vfxIceStorm: 'VFX_IceStorm.png', vfxFireStorm: 'VFX_FireStorm.png',
    rockZone: 'Rock_Zone.png', vfxSandStorm: 'VFX_SandStorm.png',
    burn: 'Burn.png', wet: 'Wet.png', stun: 'Stun.png', freeze: 'Freeze.png'
};
...
ASSETS.enemyProjectiles.badFire.src = 'BadFire_Ball.png';
ASSETS.enemyProjectiles.darkPurple.src = 'DarkPurple_Ball.png';

ASSETS.bg.src = 'Background.png'; 
ASSETS.menuBg.src = 'ManHinhCho.png';

for(let i = 1; i <= 3; i++) { let img = new Image(); img.src = `TheTree${i}.png`; ASSETS.trees.push(img); }
for(let i = 1; i <= 2; i++) { let img = new Image(); img.src = `TheRock${i}.png`; ASSETS.stones.push(img); }

const enemyNames = ['TheSlime.png', 'TheGoblin.png', 'TheSkeleton.png', 'TheBat.png', 'TheFire.png', 'TheWizard.png', 'TheOrc.png'];
enemyNames.forEach(src => { let img = new Image(); img.src = src; ASSETS.enemies.push(img); });
...
class VisualFX {
    ...
    draw(ctx) {
        ...
        if (img && img.complete && img.naturalWidth !== 0) {
            let s = 200; 
            if (this.type === 'cleave_fire' || this.type === 'cleave_normal') ctx.rotate(Math.PI / 2); 
...
class Player {
    ...
    update(dt) {
        ...
        if(MOUSE.left && this.attackTimer <= 0) {
            ...
            else if (this.weapon === 'fire_sword') { this.attackTimer = 0.35; this.meleeHit(this.dmgBase*2, 100, Math.PI, 'fire'); vfxs.push(new VisualFX(this.x, this.y, 'cleave_fire', this.facing, '#e74c3c')); SFX.slash(); } 
            else if (this.weapon === 'water_wand') { this.attackTimer = 0.4; projectiles.push(new Projectile(this.x, this.y, this.facing - 0.2, 'waterball_small', this.dmgBase*0.5)); projectiles.push(new Projectile(this.x, this.y, this.facing, 'waterball_small', this.dmgBase*0.5)); projectiles.push(new Projectile(this.x, this.y, this.facing + 0.2, 'waterball_small', this.dmgBase*0.5)); SFX.magic(); } 
            else { 
                this.attackTimer = 0.4; 
                this.meleeHit(this.dmgBase, 90, Math.PI/1.2, 'none'); 
...
class Projectile {
    constructor(x, y, a, pType, dmg) {
        ...
        else if(pType === 'tidal_wave') { this.vx = Math.cos(a)*400; this.vy = Math.sin(a)*400; this.life = 1.5; this.r = 50; this.color = '#3498db'; }
    ...
    draw(ctx) { 
        ...
        if (img && img.complete && img.naturalWidth !== 0) {
            let s = this.r * 2.5; 
            if(this.pType === 'tidal_wave') s = this.r * 4;
...
function drawInfiniteWorld(ctx) { ctx.fillStyle = '#1e272e'; ctx.fillRect(player.x - width, player.y - height, width * 2, height * 2); if (ASSETS.bg.complete && ASSETS.bg.naturalWidth !== 0) { ctx.save(); let bgScale = 0.3; ctx.scale(bgScale, bgScale); ctx.fillStyle = ctx.createPattern(ASSETS.bg, 'repeat'); ctx.fillRect((player.x - width) / bgScale, (player.y - height) / bgScale, (width * 2) / bgScale, (height * 2) / bgScale); ctx.restore(); } }
...
function loop(timestamp) {
    ...
    if(gameState === STATE.PLAY) {
        ...
        ctx.fillStyle = '#000'; ctx.fillRect(0,0,width,height); ctx.save(); let cx = width/2 - (isNaN(player.x) ? 0 : player.x); let cy = height/2 - (isNaN(player.y) ? 0 : player.y); if(shakeTime > 0) { shakeTime -= dt; if(enableShake) { cx += (Math.random()-0.5)*shakeIntensity; cy += (Math.random()-0.5)*shakeIntensity; } } ctx.translate(cx, cy); drawInfiniteWorld(ctx);
